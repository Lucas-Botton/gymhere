...
// Renders a coach's public fiche exactly the same way whether it's reached
// via search (app/coach/[id].tsx) or as a coach's own live preview of their
// draft (app/coach-preview.tsx) — one component, so the two can never drift
// apart and a coach always sees precisely what a member would see.
export default function CoachDetailView({ coach, previewMode = false }: { coach: Coach; previewMode?: boolean }) {
  const [shareOpen, setShareOpen] = useState(false);
  const [openOfferIdx, setOpenOfferIdx] = useState<number | null>(coach.offers.findIndex((o) => o.highlight));
  const isFav = useApp((s) => s.favCoaches.includes(coach.id));
        ...
        <View style={styles.body}>
          ...
          <Text weight="bold" color={colors.textMuted} style={{ fontSize: 13, marginTop: 4 }}>
            {coach.reviews > 0 ? (
              ...
              <>Nouveau sur gymhere · {coach.zone}</>
            )}
          </Text>
          ...
          {coach.diplomas.length > 0 || coach.certifs.length > 0 ? (
            <>
